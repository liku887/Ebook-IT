...
<template>
  <aside class="w-64 flex-shrink-0 h-full bg-white border-r border-slate-200 flex flex-col">
    ...
    <!-- 底部 -->
    <div class="p-3 border-t border-slate-200 flex items-center justify-between relative">
      <span class="text-xs text-slate-400">版本 v{{ catalog.version }}</span>
      <!-- AI 配置入口 -->
      <button
        @click="showAiConfig = true"
        class="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center shadow-md hover:bg-blue-700 transition"
        :title="isAiReady() ? '大模型 API 已就绪（点击修改配置）' : '配置大模型 API'"
      >
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
        </svg>
        <span v-if="isAiReady()" class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-green-400 border-2 border-white rounded-full"></span>
      </button>
    </div>
    ...
    <!-- AI 配置面板 -->
    <AiConfigPanel :show="showAiConfig" @close="showAiConfig = false" />
  </aside>
</template>
